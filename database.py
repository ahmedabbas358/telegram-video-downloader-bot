...
class DatabaseManager:
    ...
    async def update_download_status(self, download_id: int, status: str, **kwargs):
        ...
        async with self.get_session() as session:
            ...
            await session.execute(
                update(Download).where(Download.id == download_id).values(**update_data)
            )
            await session.commit()
    ...
    async def update_playlist_progress(self, playlist_id: int, completed: int = 0, failed: int = 0):
        """تحديث تقدم تنزيل قائمة التشغيل"""
        async with self.get_session() as session:
            await session.execute(
    ...
    async def get_user_stats(self, user_id: int) -> Dict[str, Any]:
        ...
        async with self.get_session() as session:
            ...
            # حجم البيانات المحملة (بصيغة إنسانية)
            size_mb = user.total_size_downloaded / (1024 * 1024)
            if size_mb < 1024:
                size_str = f"{size_mb:.1f} ميجابايت"
            else:
                size_str = f"{size_mb / 1024:.1f} جيجابايت"
