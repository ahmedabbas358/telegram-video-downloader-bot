...
def _env_int(key: str, default: int) -> int:
    """قراءة رقمية آمنة من متغيرات البيئة مع قيمة افتراضية."""
    v = os.getenv(key)
    if v is None:
        return default
    try:
...
class Config:
    ...
    # الحدود والقيود (بالميجابايت لِـ MAX_FILE_SIZE)
    MAX_FILE_SIZE: int = _env_int("MAX_FILE_SIZE", 2000)          # MB
    MAX_PLAYLIST_SIZE: int = _env_int("MAX_PLAYLIST_SIZE", 50)    # عدد الفيديوهات
    ...
    DOWNLOAD_TIMEOUT: int = _env_int("DOWNLOAD_TIMEOUT", 3600)  # ثانية
    REQUEST_TIMEOUT: int = _env_int("REQUEST_TIMEOUT", 30)      # ثانية
...
📊 **الحدود:**
• حجم ملف أقصى: {MAX_FILE_SIZE} ميجابايت
• عدد فيديوهات قائمة التشغيل: {MAX_PLAYLIST_SIZE} فيديو
        """
